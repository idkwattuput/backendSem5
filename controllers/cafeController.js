...
const createNewCafe = asyncHandler(async (req, res) => {
    const { cafe_name, cafe_location, seller_id, description, cafe_image_url } = req.body;
    const open = true;
    ...
    if (newCafe.rows.length > 0) {
        res.status(201).json({ message: `New user ${cafe_name} created` });
    } else {
...
const updateCafe = asyncHandler(async (req, res) => {
    const { cafe_id, cafe_name, cafe_location, description, cafe_image_url, is_open } = req.body;

	// Confirm data
	if (!cafe_name || !cafe_location || !description || !cafe_image_url || !is_open) {
		return res.status(400).json({ message: 'All fields are required' });
	}
	...
	// Update the user
	const updateCafeQuery = 'UPDATE Cafe SET cafe_name = $1, cafe_location = $2, description = $3, cafe_image_url = $4, is_open = $5 WHERE cafe_id = $6 RETURNING *';
	const updatedCafe = await pool.query(updateCafeQuery, [cafe_name, cafe_location, description, cafe_image_url, is_open, cafe_id]);
